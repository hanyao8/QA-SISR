...
for i in range(len(img_lr_file)):
    ...
    start_time = time.time()
    img_sr_y,img_sr_unc = ScSR(img_lr_y, img_hr_y.shape, upscale, Dh, Dl, lmbd, overlap, quantum_objects)
...
##########################################################################################
    ...
    if config.sc_algo=="qubo_bsc_dwave2":
        ...
        imsave(os.path.join(*[config.output_dir,'%04d_SR_UNC.png'%i]), img_sr_unc)
