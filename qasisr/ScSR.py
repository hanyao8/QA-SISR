...
def ScSR(img_lr_y, size, upscale, Dh, Dl, lmbd, overlap, quantum_objects):
    ...
    if config.sc_algo=="qubo_bsc_dwave1": #Quantum Annealing (Hybrid Solvers), submit to DWave hybrid solver
    ...
    elif config.sc_algo=="qubo_bsc_dwave2": #Quantum Annealing (Direct Solvers), submit to Dwave pure quantum solver
    ...
    for m in tqdm(range(0, len(gridx))):
        for n in range(0, len(gridy)):
            ...
            if config.sc_algo=="sklearn_lasso": #Lasso Regression
                ...
                total_opt_time += time.time()-opt_time_start
            elif config.sc_algo=="qubo_bsc_dwave1": #Quantum Annealing (Hybrid Solvers)
                w = flattened_m[count*Dl.shape[1]:(count+1)*Dl.shape[1]]*config.bsc_mu
            elif config.sc_algo=="qubo_bsc_dwave2": #Quantum Annealing (Direct Solvers)
                w = flattened_m[count*Dl.shape[1]:(count+1)*Dl.shape[1]]*config.bsc_mu
